...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    req.session.user = {
      ...
      type: userType,
      age: account.age, // <-- THE FIX IS HERE
    };
...
app.get("/forgot.html", (req, res) => {
  // This sends the new HTML file you just created
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    ...
    let phoneField = 'phone'; // Field name for User model

    if (!account) {
      account = await Agencies.findOne({ email: email });
      phoneField = 'agencyMobile'; // Field name for Agencies model
    }
    ...
    // --- SECURITY CHECK ---
    // Check if the provided phone number matches the correct field
    if (account[phoneField] !== phone) {
      return res.status(400).json({ success: false, message: "Email and phone number do not match." });
    }

    // --- PASSWORD UPDATE ---
    // Since you are not hashing, we just replace the plain text password.
...
app.put("/api/profile", async (req, res) => {
  ...
  // FIX: Removed 'email' from this line. It will not be read from the request.
  const { name, phone, age, currentPassword, newPassword } = req.body;
  const userId = req.session.user.id;
  const userType = req.session.user.type;
  const phoneRegex = /^\d{10}$/; // This regex matches exactly 10 digits

  // If a phone number is provided (it's not empty) AND it doesn't match the 10-digit regex
  if (phone && phone.length > 0 && !phoneRegex.test(phone)) {
    return res
      .status(400) // 400 means "Bad Request"
      .json({ message: "Phone number must be exactly 10 digits." });
  }
  try {
    // 3. Find the correct account (User or Agency)
    let account;
    let Model;
    ...
    account = await Model.findById(userId);
    if (!account) {
      return res.status(404).json({ message: "User account not found." });
    }

    // 4. Handle Password Change (Plain Text Check)
    ...
    // 5. Update the rest of the account details
    if (userType === "customer") {
      account.username = name;
    }

    // FIX: The line 'account.email = email;' has been DELETED.
    ...
    // 6. IMPORTANT: Update the session data
    req.session.user.name = (userType === "customer") ? account.username : "Agency";
    // Email in session is still correct, as it was never changed.
    req.session.user.email = account.email;
    req.session.user.phone = account.phone;
    req.session.user.age = account.age;
...
app.get("/api/recent-bookings", async (req, res) => {
  try {
    ...
    // 1. Get name AND email from session
    const { name: customerName, email: customerEmail } = req.session.user;

    // 2. THIS IS THE FIX:
    //    Find bookings where 'customerEmail' matches the email in the session
    const bookings = await Booking.find({ customerEmail: customerEmail })
      .sort({ createdAt: -1 }) // Sort by when it was created (newest first)
      .limit(5) // Get only the 5 most recent
      .lean();

    // 3. Send the bookings back
...
// ... Make sure Agencies, Vehicle, etc. are required at the top ...
...
app.get("/api/my-bookings", async (req, res) => {
  ...
  try {
    const { email: customerEmail } = req.session.user;
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    let {
      ...
      discount // <-- CHANGE #1: Read the discount from the request
    } = req.body;
    ...
    const booking = new Booking({
      ...
      agencyId,
      vehicleId,
      fare: parseFloat(fare) || 0, // This is already the FINAL (discounted) fare
      
      // --- CHANGE #2: Add the discount amount here ---
...
app.post("/api/bookings/cancel", async (req, res) => {
  ...
  try {
    ...
    // --- TIME VALIDATION ---
    if (status === "approved") {
      // combine booking.date + stations[0].time
      const dateStr = booking.date; // "2025-11-09"
      const timeStr = booking.stations[0]?.time; // "17:11"

      if (!dateStr || !timeStr) {
        return res.status(400).json({ success: false, message: "Missing date or time in booking." });
      }

      // create a valid Date object like "2025-11-09T17:11:00"
      const rideStart = new Date(`${dateStr}T${timeStr}:00`);
      const now = new Date();

      const diffMs = rideStart - now; // milliseconds until departure
      const thirtyMinutes = 30 * 60 * 1000; // 30 minutes

      if (diffMs <= thirtyMinutes) {
        return res.status(400).json({
          success: false,
          message: "Cannot cancel the ride within 30 minutes of the scheduled start time.",
        });
      }
    }
